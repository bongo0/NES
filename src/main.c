...
int main(int argc, char **argv) {
  ...
  struct nk_image pattern_table_img =
      GUI_image_rgba(&gui_ctx, nes.ppu.pattern_table, 128 * 2, 128);
  ...
  while (!close) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_KEYDOWN:
        ...
        case SDLK_f:
          while(!nes.ppu.frame_complete){BUS_tick(&nes);}
    ...
    if (nk_begin(gui_ctx.nk_ctx, "pattern table",
      ...
      GUI_image_refresh(&gui_ctx, &pattern_table_img, nes.ppu.pattern_table,
                        128 * 2, 128);
    ...
    if (nk_begin(gui_ctx.nk_ctx,
                 ...
                 "0D  0E  0F",
                 nk_rect(50, 400, 530, 380),
                 NK_WINDOW_TITLE | NK_WINDOW_BORDER | NK_WINDOW_MOVABLE |
                     NK_WINDOW_SCALABLE)) {
      GUI_memory_view(&gui_ctx, nes.ram, 2048);
    }
    nk_end(gui_ctx.nk_ctx);
